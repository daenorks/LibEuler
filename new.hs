...
combinaison [] = []
combinaison (x:xs) = (map (x*) xs) ++ (combinaison xs)
...
--problem42
p42 wor = length $ filter (`elem` nn) $ map sum $ map (map (\x -> (ord x) - 64)) wor

divnumtri n = product $ map (+1) $ map length $ L.group $ sort $ delete 2 $ primeF (n) ++ primeF (n + 1)
...
tripletPythagore = [[x,y,z]| x <- [1..], y <- [1..x], z <- [1..y], x^2 == (y^2 + z^2)]
...
primeUntilH n (x:xs) = if x > n
        then (x:xs)
        else (x:) $ primeUntilH n ([y | y <- xs, (mod y x) /= 0])

maxfprime (a,b) list = maxfprimeh (\x -> x^2 + a*x + b) list 0
maxfprimeh f list n = if elem (f n) list
        then maxfprimeh f list (n+1)
        else n
...
truncPrimes = take 11 $ filter (>10) primess
    where primess = filter (\x -> elemList (multTailsInt x) (takeWhile (<= x) primes)) primes


lrprimes = lrprimesh ([2,3,5,7]) (7*11)
lrprimesh list maxi = if newx == Nothing
        then list
        else let justx = (\(Just x) -> x) newx in lrprimesh (justx:list) (justx*100)
    where tai x = [mod x (10^((sizeInt x) -1)),div x 10]
          primess = filter (> (head list)) $ takeWhile (<= maxi) primes
          newx = find (\x -> elemList (tai x) list) primess
...
cycleMax n = find (\x -> (fromIntegral $ length(ncycle x)) == (x-1)) $ reverse $ primeUntil n
...
isAmicable number = (number2 /= number) && (number == (sum $ init $ listDiv number2))
    where number2 = (sum $ init $ listDiv number)

resilientR number = (fromInteger(resilient number))/(fromInteger(number -1))
resilient number = (number -1 -) $ sum $ map (\(x,y) -> (-1)^(x+1) * (div (number-1) y)) $ map (\x -> (mod (length x) 2, product x)) $ map primeF $ delete 1 $ listDiv $ product $ uniqPrime number
--cube = [(digs x^3


prime4digit = filter (>1000) $ primeUntil 10000

threeprimeseq = primeseq prime4digit
primeseq [] = []
primeseq (x:xs) = if elem elem1 xs && elem elem2 xs
        then x:(primeseq xs)
        else primeseq xs
    where elem1 = (mod x 1000)*10 + (div x 1000)
          elem2 = (mod x 100)*100 + (div x 100)
